...
)
{
    ...
    {                       
        ...
        *(strrchr(ModuleFileName, '\\') + 1) = 0;
        ...
        GetPrivateProfileStringA("Extensions", "ext", "HTML,TXT,URL", strExtensions,1024, iniPath.c_str());
...
CefString GetLoadUrl(char* FileToLoad) {
    ...
    if (sFileExt=="URL") {
        char strUrl[2048];
        GetPrivateProfileStringA("InternetShortcut", "URL", "https://www.baidu.com", strUrl, 1024, FileToLoad);
        wstring wsLoadUrl = ANSIToUnicode(strUrl);
        target_url = CefString(wsLoadUrl);
    }
    else if (sFileExt == "TXT") {
        wstring wsLoadUrl = ANSIToUnicode(FileToLoad);
        target_url = CefString(wsLoadUrl);
    }
    else {
        ...
        if (sLoadUrl == "") {
            ...
            sLoadUrl = sLoadUrl.replace(sLoadUrl.find(pattern), pattern.length(), FileToLoad);
            wstring wsLoadUrl = ANSIToUnicode(sLoadUrl);
            target_url = CefString(wsLoadUrl);// + "?" + FileToLoad
