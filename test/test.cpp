...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   //加载DLL文件
   h = LoadLibrary(L"wlx_cef.wlx");
   
   
   //定义一个函数指针类型ListLoad(HWND ParentWin, char* FileToLoad, int ShowFlags)
   typedef HWND(__stdcall *PFUNC)(HWND, char*, int);
   typedef void (__stdcall *PFUNC2)(char*, int);
   //通过函数名称来获取函数地址
   PFUNC pListLoad = (PFUNC)GetProcAddress(h, "ListLoad");

   
   PFUNC2 ListGetDetectString =(PFUNC2)GetProcAddress(h, "ListGetDetectString");
   //调用函数
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        {
            ...
            case IDM_EXIT:
            {
                typedef void(__stdcall* PFUNC3)(HWND, HWND, char*, int);
                PFUNC3 ListLoadNext = (PFUNC3)GetProcAddress(h, "ListLoadNext");
                ListLoadNext(hWnd,hWndChild,NULL,1);
            }
                break;
            default:
    ...
    case WM_DESTROY:
    {
        //MessageBox(NULL, L"WM_DESTROY", TEXT("test"), MB_OK);
        //释放DLL文件
